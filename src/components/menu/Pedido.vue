    ...
    <div class="text-left">
        ...
        <v-dialog v-model="dialog" width="auto">
            <v-card :width="widthDisplay">
                ...
                <v-card-subtitle v-if="categoria === 'Almuerzo'"
                    >Contornos disponibles a elegir:
                    <strong>{{ contornosDisponibles }}</strong></v-card-subtitle
                >
                <v-card-text class="py-0">
                    <v-container>
                        <v-row>
                            <v-col
                                v-if="categoria != 'Delivery'"
                                cols="6"
                                class="pa-0"
                            >
                            ...
                            <v-col v-if="categoria === 'Almuerzo'">
                                <v-sheet rounded color="yellow-darken-2">
                                    <h5>Nota:</h5>
                                    <h6>
                                        Producto que no indique sus contornos a
                                        excepcion del Pabellon criollo y la
                                        Pasta a la bologna, seran servidos con
                                        arroz, ensalada cocida y tajadas
                                    </h6>
                                </v-sheet>
                            </v-col>
                        </v-row>
                        ...
                        <v-row v-if="categoria != 'Delivery'">
                            <v-col class="pb-0">
                                <v-sheet rounded color="yellow-darken-2">
                                    <h3>
                                        {{ pedido.cantidad }} {{ menuOpcion }}
                                        {{ concatenarContornosSeleccionados }}
                                    </h3>
                                </v-sheet>
                            </v-col>
                        </v-row>
                        ...
                        <Aviso
                            ...
                            @activar-aviso="
                                () =>
                                    (propsAvisoMaxProductos.activarAviso = false)
                            "
                ...
                <v-card-actions color="yellow-darken-2">
                    ...
                    <v-btn
                        ...
                        color="green"
                        @click="guardarEnListaPedidos(pedido)"
                        >Guardar pedido</v-btn
                    >
                    <v-btn
                        size="x-small"
                        v-show="descripcion.length > 0"
...
const widthDisplay = computed<string>(() => {
    //controla el ancho del component card dependiendo del dispositivo
    if (isMobile.value) {
        return "350";
    } else {
        return "600";
...
const colsContornos = computed<number>(() => {
    //establece las columnas del componente switch de loss contornos dependiendo del dispositivo
    if (isMobile.value) {
        return 6;
    } else {
        return 4;
...
listaContornos = listaMenu.value.filter(
    (producto: Producto) =>
        producto.categoria === "Contorno" && producto.disponible === true
); // filtro para obtener de la lista solo los productos con categoria contorno
...
const resetearSeleccion = (): void => {
    //resetea los valores de los contornos seleccionados por el cliente
    descripcion.value = [];
};
...
const guardarEnListaPedidos = (pedido: Pedido): void => {
    let cantidadProductos: number = lista.listaPedidos.reduce(
        (acum, producto) => {
            return acum + producto.cantidad;
        },
        0
    ); // suma las cantidades de cada producto de la lista
    ...
    if (totalCantidadProductos > 10) {
        // Verifica si la lista tiene menos de 10 productos, si sobrepasa el limite mostrara al usuario que ha cubierto el limite de productos a pedir
        propsAvisoMaxProductos.activarAviso = true;
        propsAvisoMaxProductos.mensaje =
            "Usted supera el limite de 10 productos en su lista de pedidos...";
    } else {
        let pedidoOriginal: Pedido = {
            //Elimina la reactividad del pedido
